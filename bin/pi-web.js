...
// Read distDir from next.config.ts if available
let nextDir = path.join(pkgDir, ".next");
try {
  const configPath = path.join(pkgDir, "next.config.ts");
  if (fs.existsSync(configPath)) {
    const configContent = fs.readFileSync(configPath, "utf8");
    const match = configContent.match(/distDir\s*:\s*["']([^"']+)["']/);
    if (match) {
      nextDir = path.join(pkgDir, match[1]);
    }
  }
} catch {
