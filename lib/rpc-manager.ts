  ...
  async send(command: Record<string, unknown>): Promise<unknown> {
    ...
    switch (type) {
      case "prompt": {
        ...
        console.log("[RPC] prompt command, message:", command.message, "images count:", promptImages?.length ?? 0);
        this.inner.prompt(command.message as string, promptImages?.length ? { images: promptImages } : undefined).catch((err) => {
          console.error("[RPC] Agent prompt failed:", err);
        });
        return null;
      }
...
export async function startRpcSession(
  ...
  const starting = (async () => {
    ...
    console.log("[RPC] Session created, model:", inner.model?.provider, "/", inner.model?.id);
    console.log("[RPC] Session file:", inner.sessionFile);
